...
int main(void)
{
  ...
  //Have parent read from second pipe
  while(1){
    Read(fd2[READ_END], (void*) &read_msg2[a],1);
	 printf("read2 %c \n", read_msg2[a]);
	 if (read_msg2[a] == 0){
	   break;
	 }
  ...
  //Close the read end of the pipe
   ...
   close(fd[READ_END]);
    ...
    while (write_msg2[b] != 0){
    Write(fd2[WRITE_END],(void*) &write_msg2[b], 1);
    // write(fd[WRITE_END],"\n" , 1);    //test to see if writing 1 char at a time
    b++;
    }
    /* close the write end of the pipe */
