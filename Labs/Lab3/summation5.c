...
void *SumExp(void *args_ptr) {
  ...
  int startTimet = startTime.tv_sec;
  int startTimeU = startTime.tv_usec;
  ...
  int endTimet = endTime.tv_sec;
  int endTimeU = endTime.tv_usec;
  int elapsedS = endTimet - startTimet;
  int elapsedU = endTimeU - startTimeU;
  printf("Elapsed time of thread %d is %d seconds and %d useconds \n", myargs_ptr ->tid, elapsedS, elapsedU);
  time_t timeStr = endTime.tv_sec;
  fflush(stdout);
  printf("Current time of thread %d is %s \n", myargs_ptr->tid, ctime(&timeStr));
